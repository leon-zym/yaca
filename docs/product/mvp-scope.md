...
- Register, list, select, and remove Workspace registrations.
- List, create, activate, inspect, rename, and recoverably trash Sessions.
...
- Stop the active Run and wait for the runtime to become idle.
...
- Reconcile realtime state with a bounded Host snapshot after refresh or connection loss.
...
- Never automatically replay a command whose delivery is unknown.
...
6. Supported Thinking Levels are derived from actual model metadata or validated runtime behavior; the UI does not advertise unsupported levels.
7. Full read and shell output can be ingested before any SDK-provided `fullOutputPath` expires; if it cannot, the affected complete-content path is a release blocker.
8. Edit/write event details provide sufficient patch or diff data for the dedicated view; otherwise the Host must derive it from observable Workspace state through a tested path.
...
- Playwright covers the required vertical path, Stop, model/Thinking timing, refresh, forced connection loss, and Host restart;
